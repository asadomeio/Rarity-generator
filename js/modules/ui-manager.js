...
function UIManager(document) {
    ...
    function createItemEntry(item) {
        ...
        const npcLinks = (item.npcs || (item.npcIDs ? item.npcIDs.map(id => ({id})) : [])).map(npc => 
            `<a href="https://www.wowhead.com/npc=${npc.id}">${npc.name || `NPC #${npc.id}`}</a>`
        ).join(', ');
        const chanceText = item.chance ? ` (Chance: 1 in ${item.chance})` : '';
        itemDiv.innerHTML = `<div class="item-info"><span>[${item.type}] ${itemLink}</span><small>From: ${npcLinks}${chanceText}</small></div>`;
        return itemDiv;
    }
